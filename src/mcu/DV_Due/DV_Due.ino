...
//--------------Send spikes-------------//  
  ...
  int IN_ack = digitalRead(10);// cread the ACK IN pin
  ...
  if(previous_level == current_level)
  {
    ...
    if(ind_Num==0)
    {
      ...
      {
        sendSpike(current_level); // SEND SPIKE
...
//--------------Receive spikes-------------// 
  ...
  */
  ...
  // initial Num_com == 10
        ...
        {
          Num_com = 0;
          ARMpos = mostFrequent(com,10); //Chose position by the most frequent x pos
          switch(ARMpos)
          {
            case 0:
              myservo.writeMicroseconds(1100);
...
int mostFrequent(int vet[], int n)
{
  ...
  int res = 0;
   ...
   for(int i=0; i<8; i++){ // for each final position 0 - 7
      for(int k=0; k<n; k++){
        if(vet[k]==i){
          count++;
        }
